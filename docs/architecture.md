...
The plugin resolves one controller/device rendering profile and sends its optional bounded `imageSize` with existing metadata; Lua image helpers consume that value rather than maintaining a second device map. Keypad profiles use the documented native 72px Stream Deck, 80px Mini, 96px XL/Neo, and 120px Plus sizes. The displayless Pedal and devices whose official SDK documentation does not publish per-key dimensions retain the documented 72px fallback; missing, malformed, unknown, or controller-mismatched metadata does too. Recognized encoder profiles use the SDK's 48×48 icon slot: title-only LCD feedback uses built-in `$A1`, a valid `value`/`indicator` pair uses `$B1`, and other decorations use `$A0` with its deterministic 200×100 per-encoder full-canvas image.
