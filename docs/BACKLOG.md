...
**Status:** Blocked on companion product/lifecycle approval 2026-07-21 — B3-T1, B3-T2, and B3-T2.5 are complete; B3-T3 remains unstarted. A second architecture assessment confirmed that the repository cannot safely select the companion footprint or native Keychain/TLS boundary without that approval. `29c873181695eafc5d7deac9929e8f45e28a5823` and `9912b073fb4b8f6cad2eab7fa0a8daed2d7752bf` confine the legacy token transport to literal loopback URLs, including rejection of normalized numeric/IP aliases; `bun test tests/bridge.test.ts`, `bun run check`, and `bun run build` passed after the fix.
...
- [x] B3-T2 — Define credential provisioning for a second machine plus rotation, migration, failure, and fallback behavior without weakening v1 authentication.
  - Decision (2026-07-21): the primary companion generates and retains its private CA and named server key in the macOS Keychain; each remote plugin generates a non-exportable client key in that machine's Keychain and creates a CSR. Enrollment is an explicit administrator ceremony: transfer the CSR by any channel, compare its SHA-256 fingerprint against a separately observed primary-companion display, then issue a client certificate whose SAN contains a random stable client ID. Return only the signed client certificate, CA certificate, server name/port, and allowed client ID; the remote compares the CA fingerprint against a separately observed primary display before storing the public configuration, and validates the server chain and name on every connection. The primary accepts a connection only when the presented client certificate chains to its CA and its serial/client ID is still authorized; identity always comes from that verified certificate, never the enrollment file or a remote message.
  - Rotation and recovery: renew client keys with a fresh CSR; keep the old certificate authorized only until the new one successfully connects, then revoke its serial. The primary companion's authorization store is the revocation source of truth, so revocation is immediate without depending on LAN CRL delivery. Rotate the server leaf under the same CA transparently; replacing the CA, moving a remote plugin to another machine, or losing either Keychain requires a new fingerprint-verified enrollment. Companion/plugin upgrades may migrate only validated Keychain references and authorization records, never export private keys. Missing, expired, revoked, wrong-name, or untrusted credentials leave that remote profile disconnected with a safe actionable reason, clear its authenticated relay session, and require renewal or re-enrollment.
  - Rationale and compatibility: CSR/public-certificate transfer exposes no private key; independent fingerprint comparison prevents a substituted CSR, CA, or response from silently enrolling an attacker. Certificate authorization provides deterministic per-client rotation/revocation without a shared LAN secret. Remote profiles never send the v1 token and never downgrade to `ws://` or token authentication; the existing local profile remains the only loopback-token path and continues to default to `localhost`. This is a remote companion enrollment contract, not a v1 wire change; B2 remains gated unless B3-T3 introduces a breaking application protocol.
...
- [ ] B3-T2.75 — Define the companion runtime and trust bootstrap needed to ship the selected mutual-TLS architecture.
  - Blocker (2026-07-21; reconfirmed by a second architecture assessment): authentication semantics are decided, but the product/runtime boundary is not. The repository ships only an Elgato-managed Node plugin and Hammerspoon Lua module; it has no companion executable, native Keychain-backed TLS integration, macOS build/package/signing path, install/update/uninstall path, or launch owner. B3-T1/T2 already lock mutual TLS, certificate-derived identity, fingerprint-verified CSR enrollment, Keychain-held private keys, authorization/revocation, no downgrade, and authenticated exclusive local relay while Lua stays loopback-only.
  - Required decision: approve either (a) a separately installed, persistent per-user macOS companion or (b) a helper bundled and lifecycle-owned by the Stream Deck plugin, and state whether a native TLS/Keychain component may also run on the remote Mac. These alternatives materially differ in availability, signing/distribution, install/update/uninstall, Keychain access, and restart behavior. If neither footprint is acceptable, reopen B3-T1/T2 or remove the remote-LAN scope.
  - Unblock boundary: record the approved runtime/toolchain; which host(s) run it; artifact, install, launch, restart, upgrade, and uninstall owner; authenticated local-IPC contract; and enrollment/configuration surface. T2.75 completes only with a runnable topology: remote profile → mutually authenticated TLS remote hop → companion derives identity solely from the verified client certificate → authenticated exclusive local IPC → Lua. Unauthorized, revoked, expired, wrong-name, or untrusted peers must fail closed; remote profiles never send/read the v1 token or downgrade; private-key owners/access rules and actionable failure behavior must be named.
- [ ] B3-T3 — Implement coordinated TypeScript/Lua authentication changes after B3-T2.75.
