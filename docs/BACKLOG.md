...
**Status:** Complete 2026-07-21 — `27a801f7fa43c8087c3a4cecd0acf5eb5b20a518`, `4a12c21fa2ad1958c0477ee292684f374b38ad7b`, `8a376badc1b142e92ebcea71581e7f6a954ee262`, and `72990fb91eeb7d0e1386b6592b7278fd6ca5c0cb` implement and harden the opt-in PSK LAN transport. Final independent verification passed B3-T3/T4, B3-AC1 through B3-DOD1: `bun run check`, `bun run test` (116 TypeScript, 70 Lua bridge, 10 Lua startup), `bun run build`, and Stream Deck package validation passed. The reviewer reproduced and confirmed rejection of stale proof replay after a tampered frame, and confirmed source-map synchronization. B4-T5 retains the user-facing remote-profile configuration; B3 exposes only the deliberate library/manual profile. Re-verification on 2026-07-22 passed all criteria and observed one contract overclaim: "MAC failure closes the connection" is literal only for the plugin, because `hs.httpserver` gives Lua no way to close a WebSocket — Lua fails closed by retiring the session and keys instead. `eb6d67d57bfaca6060885b95483909824ed233a9` aligns the T1 decision above, `docs/security.md`, and `docs/protocol.md` with that asymmetric enforcement.
...
**Status:** Complete 2026-07-21 — `52fca380254309684c5bec2b55410dd0acbc8e3c`, `417bd8b04812106e7acce18eb7a909d8ee61d193`, `15e7b55a2b388fda987e7856ef538d5275660e5b`, and `19903281268fbce6a072716de2226d0f0afcb24d` define, implement, and harden the limits. Independent verification passed B5-AC1 through B5-DOD1; `bun run test && bun run check && bun run validate` passed. Re-verification on 2026-07-22 passed all criteria but found two claims resting on code reading alone; `fbd38a612939bc4ca77a7db5625d6d3d18953762` adds the regression tests: oversized `lanFrame` payloads are rejected before any MAC computation (instrumented HMAC call counters in both runtimes), and the real plugin backoff at its fastest jitter never depletes the unauthenticated burst-6/12-per-minute bucket across a sustained rejection storm, with steady-state cadence pinned at ≥ 5 s.
...
- [x] B5-T1 — Define maximum JSON depth, collection sizes, parse work, and buffering behavior on both sides.
...
**Status:** Complete 2026-07-21 — `8ad881bbb193f6ab63d47bb662abbde3cfa88898`, `d365c7f1b81a6000f684ee7735fd4659d886235c`, `8265acb309d4f4117c6d38cee33888332b57efed`, `2941b739571eff6365e31c5427a94fe045704fe9`, and `7f14b16efb0cf6869d07fbfcdfecea14c960788e` implement B4 and correct its first review findings. Final independent security review required a bridge-owned session-ID entropy source; `bcee18a56dd778e8d1e57c3f22ee1541996a6556` uses 32-byte CSPRNG IDs and adds the restart/stale-session regression, while `e7fbf5c6102b6fe773715e7d505e133eb5915306` and `1790e88046460761fe5de3fea5c6c0a25084953a` reconcile session-ID documentation. The verifier passed B4-AC1 through B4-DOD1 after the fix. `bun run check && bun run test` passed (118 TypeScript, 78 Lua bridge, 10 Lua startup tests). Re-verification on 2026-07-22 passed B4-AC1 through B4-AC3 but failed B4-DOD1 on one stale paragraph: the troubleshooting guide's diagnostics section still called the server loopback-only with one supported WebSocket client. `7291fc774e7da29fd7f3174ed1a48bc90adbd797` rewrites it to the slot contract (loopback default, one active application session per slot, LAN slots as separately configured broadcast domains).

**Product assessment:** The use case is a second Stream Deck attached to another computer on the LAN driving the same Hammerspoon bridge. That makes this a transport-and-security project, not a connection-count change: binding leaves loopback only as explicit opt-in (B3); abuse limits must already exist (B5). Client identity becomes per-machine, instance ownership must be partitioned per client, and one client's disconnect or malformed traffic must not disturb the other's keys. Shipping this revises the README non-goals ("multiple simultaneous plugin clients") and the loopback-only architecture boundary — deliberately, through this contract. B4-T1 selects per-client `hs.httpserver` listener slots: this preserves specific-interface binding and routes each callback-return/send path through its owner. `hs.socket` is rejected because its listener reads lack peer attribution, listener writes broadcast, and it cannot bind the configured interface.
...
- [x] B4-T1 — Define client identity, authorization ownership, session/context isolation, and lifecycle semantics.
  - Decision (2026-07-21): replace the singleton transport state with one dedicated `hs.httpserver` listener and state object for each admitted client slot: the unchanged localhost/token listener is the fixed legacy slot, and each explicitly configured LAN `clientId` owns one listener on a distinct configured port and named interface. A slot owns its immutable identity and credential, handshake state, session ID, rate buckets, frame keys/sequences, instance/context registry, response queue, and timers. Callback closures capture their slot; output uses only the owning listener, never a mutable global active transport. A LAN proof must authenticate the identity assigned to that listener; session IDs and instance IDs are scoped to the slot, and a reconnect retires only that slot's session and contexts. No lookup, callback, error, timer, rate violation, malformed frame, or disconnect may read, clear, route to, or dispatch through another slot.
  - Rationale: `hs.socket` cannot meet the contract: listener read callbacks have no peer identity, listener writes broadcast to connected clients, and `listen` cannot bind the configured interface. `hs.httpserver` gives each slot a bound interface/port and listener-scoped callback-return/send paths. B4-T2 bounds profile/listener/application-session/instance counts; B4-T3 must keep session, context, rate, and response state slot-local. The supported-runtime probe establishes that listener objects bind concurrently, a listener permits multiple WebSockets without a close callback, callback returns reach the originating peer, and `send` broadcasts to every peer of that listener. Therefore a slot is a client-identity and broadcast domain—not a per-socket channel—and the protocol, rather than listener replacement, owns the one-active-application-session rule. B4-T5 defines the bounded per-client configuration shape and user-facing opt-in behavior; default configuration still creates only the legacy localhost slot.
- [x] B4-T2 — Define resource, connection, and failure limits for concurrent clients.
  - Decision (2026-07-21): the default creates only the fixed legacy localhost slot. Explicit opt-in adds at most four LAN client slots, for a hard maximum of five `hs.httpserver` listeners and five concurrent active application sessions; every slot maintains at most 64 visible instances, so the bridge holds at most 320 contexts and at most one long-press timer per context. Listener ports, including the loopback port, must be unique; client IDs and LAN credential paths must be unique; invalid, duplicate, or over-limit profiles fail startup before a usable bridge exists. No cross-slot queue or fallback is introduced: each slot retains only its bounded B5 rate buckets, in-flight response list, and instance/timer set. A malformed frame, stale session, rate breach, callback failure, or a new authenticated handshake may retire only that slot's session, contexts, keys, sequences, timers, and pending responses; it must neither evict nor write through another slot. Startup failure stops every listener already created, and safe diagnostics identify only the slot-safe client ID and stable failure code. Probe result (2026-07-21): listener objects bind concurrently, callback returns go only to their originating peer, and `listener:send` broadcasts to every concurrent peer of that one listener. Multiple WebSockets are therefore expected inside a slot; no per-peer admission, targeted send, or close signal exists. T3 must enforce only one active *application session* per slot, scope all output and state to the slot, and preserve B3's explicit LAN-observable-content boundary rather than claiming per-WebSocket confidentiality or delivery.
- [x] B4-T3 — Implement isolated client registries and synchronization behavior.
  - Completion (2026-07-21): Slot-owned listeners retain credentials, sessions, rate buckets, response queues, contexts, and timers; callback closures and context emitters route only through their owner. The bridge creates the fixed loopback slot plus at most four configured LAN slots, stops every started slot on failure, and preserves the existing loopback contract. Tests cover isolated same-ID contexts, output routing, malformed traffic, reconnect/stale-session isolation, startup rollback, and slot configuration bounds. `bun run check && bun run test` passed.
  - Probe result (2026-07-21): on the supported Hammerspoon runtime, listener A on port 19421 and listener B on port 19422 bound concurrently. A `send` during A's sole connection reached A only; B's independent reply reached B only. A second A WebSocket then opened, stayed live with the first, and both received their own callback-return replies; Hammerspoon reported no close until test cleanup. With both A peers live, an A `push` invoked `listener:send("push-A")` and both A peers received `push-A`; the callback return reached only the initiating A peer. This proves `send` is a listener-wide broadcast and completes the required runtime probe. T3 may implement slot-owned registries and one active protocol session per slot, but must not model a slot as a single WebSocket or promise per-peer output confinement.
  - API confirmation (2026-07-21): the official `hs.httpserver:websocket(path, callback)` contract passes the callback only the received-message string and accepts only its string response; it exposes neither a connection identity nor a handshake/peer-close hook. The slot model therefore cannot reject or target an individual second WebSocket, and no further rejection probe is meaningful.
- [x] B4-T4 — Add concurrent-client, stale-session, disconnect, and resource-exhaustion tests.
  - Completion (2026-07-21): `8265acb309d4f4117c6d38cee33888332b57efed` covers loopback-only default startup; isolated same-ID contexts; malformed, stale-session, reconnect, disconnect, and 64-instance exhaustion behavior; bounded safe diagnostics; and startup admission limits. `bun run lua:check` and `lua hammerspoon/tests/run.lua` passed.
- [x] B4-T5 — Define the explicit opt-in network configuration (default remains loopback) and update the README/architecture non-goal wording as part of the same change.
  - Completion (2026-07-21): `8265acb309d4f4117c6d38cee33888332b57efed` documents the bounded per-client `lan.clients` form, single-client shorthand, five-listener limit, unique identity/port/key requirements, and loopback default across README, architecture, protocol, Lua API, security, and troubleshooting guides. `2941b739571eff6365e31c5427a94fe045704fe9` corrects the slot-vs-WebSocket lifecycle contract; `7f14b16efb0cf6869d07fbfcdfecea14c960788e` verifies diagnostics against raw output.

#### Acceptance criteria

- [x] B4-AC1 — One client's disconnect, malformed traffic, or stale session cannot dispatch against another client's contexts.
- [x] B4-AC2 — Client admission and resource limits are bounded and observable through safe diagnostics.
- [x] B4-AC3 — Existing single-client loopback behavior remains the compatible default; LAN operation requires explicit configuration.

#### Definition of Done

- [x] B4-DOD1 — Multi-client behavior is documented as a protocol/architecture contract and independently security-reviewed.
