...
**Status:** In progress 2026-07-21 — B4-T1/T2 define isolated slots and finite bounds. B4-T3 is paused on the required real-Hammerspoon listener probe; B4-T4/T5 remain.
...
- [x] B4-T2 — Define resource, connection, and failure limits for concurrent clients.
  - Decision (2026-07-21): the default creates only the fixed legacy localhost slot. Explicit opt-in adds at most four LAN client slots, for a hard maximum of five `hs.httpserver` listeners and five concurrent authenticated sessions; every slot admits exactly one WebSocket connection/session and maintains at most 64 visible instances, so the bridge holds at most 320 contexts and at most one long-press timer per context. Listener ports, including the loopback port, must be unique; client IDs and LAN credential paths must be unique; invalid, duplicate, or over-limit profiles fail startup before a usable bridge exists. No cross-slot queue or fallback is introduced: each slot retains only its bounded B5 rate buckets, in-flight response list, and instance/timer set. A second connection, handshake, malformed frame, stale session, rate breach, callback failure, or disconnect may retire only that slot's session, contexts, keys, sequences, timers, and pending responses; it must neither evict nor write through another slot. Startup failure stops every listener already created, and safe diagnostics identify only the slot-safe client ID and stable failure code. B4-T3 must empirically establish the exact `hs.httpserver` second-WebSocket/replacement behavior and map it to this one-session rule without relying on a close callback.
- [ ] B4-T3 — Implement isolated client registries and synchronization behavior.
  - Manual gate (2026-07-21): before changing the singleton transport, verify on the supported Hammerspoon runtime whether two independently configured `hs.httpserver` objects can concurrently bind distinct interface/port pairs, each confines `send` to its own WebSocket peer, and a second WebSocket to one object replaces, rejects, or coexists without a close callback. An isolated T3 refactor attempt produced no commit; its focused fake-runtime tests cannot establish those runtime semantics. The next pass resumes from this probe result.
