...
**Status:** Blocked pending companion runtime and trust-bootstrap decision 2026-07-21 — B3-T1, B3-T2, and B3-T2.5 are complete; B3-T3 remains unstarted. `29c873181695eafc5d7deac9929e8f45e28a5823` confines the legacy token transport to literal loopback URLs; `bun test tests/bridge.test.ts`, `bun run check`, and `bun run build` passed.
...
- [x] B3-T2.5 — Restrict the legacy token transport to literal loopback endpoints before opening a socket or reading the shared token.
  - Completion (2026-07-21): `29c873181695eafc5d7deac9929e8f45e28a5823` accepts only `ws://localhost`, `ws://127.0.0.1`, or `ws://[::1]` and rejects all other legacy endpoints before token authentication. The source, compiled plugin artifact, tests, and security guide agree.
- [ ] B3-T2.75 — Define the companion runtime and trust bootstrap needed to ship the selected mutual-TLS architecture.
  - Blocker (2026-07-21): the repository has no native companion target or runnable build/distribution/lifecycle baseline. Before B3-T3, approve the executable, build/distribution and launch owner; remote-listener-to-localhost proxy topology; client/server identity issuance or pairing; Keychain ownership; rotation/revocation; plugin trust/configuration; and an end-to-end acceptance scenario proving mutual TLS on the remote hop while Lua remains loopback-only. A partial terminator would be unusable or weaken the v1 boundary.
