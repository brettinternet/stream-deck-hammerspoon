...
**Status:** In progress 2026-07-21 — B4-T1/T2 define intended isolated slots and finite bounds. The real-Hammerspoon probe shows one `hs.httpserver` listener accepts concurrent WebSockets without a close callback, so B4-T3 needs an enforceable per-slot admission design; B4-T4/T5 remain.
...
- [x] B4-T1 — Define client identity, authorization ownership, session/context isolation, and lifecycle semantics.
  ...
  - Rationale: `hs.socket` cannot meet the contract: listener read callbacks have no peer identity, listener writes broadcast to connected clients, and `listen` cannot bind the configured interface. `hs.httpserver` gives each slot a bound interface/port and a listener-scoped callback-return/send path. B4-T2 must bound profile/listener/session/instance counts and enforce one active connection per slot; B4-T3 must empirically verify concurrent `hs.httpserver` binding, per-object send confinement, second-WebSocket behavior, and replacement/isolation without a close callback. B4-T5 defines the bounded per-client configuration shape and user-facing opt-in behavior; default configuration still creates only the legacy localhost slot.
- [x] B4-T2 — Define resource, connection, and failure limits for concurrent clients.
  - Decision (2026-07-21): the default creates only the fixed legacy localhost slot. Explicit opt-in adds at most four LAN client slots, for a hard maximum of five `hs.httpserver` listeners and five concurrent application sessions; each slot must expose one active application session and maintains at most 64 visible instances, so the bridge holds at most 320 contexts and at most one long-press timer per context. Listener ports, including the loopback port, must be unique; client IDs and LAN credential paths must be unique; invalid, duplicate, or over-limit profiles fail startup before a usable bridge exists. No cross-slot queue or fallback is introduced: each slot retains only its bounded B5 rate buckets, in-flight response list, and instance/timer set. A malformed frame, stale session, rate breach, callback failure, or disconnect may retire only that slot's session, contexts, keys, sequences, timers, and pending responses; it must neither evict nor write through another slot. Startup failure stops every listener already created, and safe diagnostics identify only the slot-safe client ID and stable failure code. Probe result (2026-07-21): two listener objects bound concurrently and their independent clients received only their own `send`/callback output, but a second WebSocket to one listener coexisted with the first and neither client closed. Therefore T3 must not assume listener-level replace/reject behavior or a close callback; it must first measure `listener:send` while both peers are live and establish an enforceable application-session admission model.
- [ ] B4-T3 — Implement isolated client registries and synchronization behavior.
  - Probe result (2026-07-21): on the supported Hammerspoon runtime, listener A on port 19421 and listener B on port 19422 bound concurrently. A `send` during A's sole connection reached A only; B's independent reply reached B only. A second A WebSocket then opened, stayed live with the first, and both received their own callback-return replies; Hammerspoon reported five opens and no close until test cleanup. This proves independent listener objects coexist and that a listener does not replace/reject a second peer. It does not establish where `listener:send` routes when two peers share that listener. Until that is measured, the callback API has no demonstrated per-peer admission/eviction signal, so T3 cannot truthfully enforce the one-application-session-per-slot contract.
