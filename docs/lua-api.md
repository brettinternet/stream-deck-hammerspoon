...
The callbacks receive the current context. `press`, `release`, `appear`, and `disappear` return values are ignored. `appearance` must return:
...
- `release(context)` runs only for a key-up event. It is optional; when absent, the release event is ignored after instance/action validation. Its return value is ignored.
