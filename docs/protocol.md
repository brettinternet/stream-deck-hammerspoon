...
- `sessionId` is generated by Lua from 32 CSPRNG bytes read from `/dev/urandom` for each accepted `hello`. It identifies only the current authenticated WebSocket session; it is never logged or persisted.
...
4. Each valid `hello`, including one received while already authenticated, is accepted after token/version checks. Lua safely clears only that slot's prior instance contexts, generates a fresh non-empty opaque `sessionId` from 32 CSPRNG bytes read from `/dev/urandom`, and returns it in `helloAck.sessionId`.
