...
3. Confirm the plugin receives `helloAck.sessionId`, keeps it only in memory, and includes it on `listActions`, each lifecycle event, `keyDown`, `keyUp`, `dialDown`, `dialRotate`, `dialUp`, and `requestAppearance`.
