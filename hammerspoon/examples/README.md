...
| [`application.lua`](application.lua) | `com.brettinternet.hammerspoon.application-toggle` | Opens a configured app when needed, hides/shows it on a tap, and closes it on a long press. Without a bundle ID, it follows the frontmost app. | Uses `hs.application`. Optionally set an application bundle ID in the action settings. |
...
| [`youtube.lua`](youtube.lua) | `com.brettinternet.hammerspoon.youtube` | Plays or pauses the first YouTube video tab in Chromium, or opens a configured YouTube URL when no video tab exists. | Uses Chromium's Hammerspoon JavaScript automation and `hs.eventtap`. Allow Hammerspoon to automate Chromium. |
