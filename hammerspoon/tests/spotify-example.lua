return function(test, load_fixture, context, assertTrue, _, assertEqual)
  test("Spotify example controls playback, volume, tracks, and album artwork", function()
    ...
    local spotify = {
      ...
      getCurrentTrackArtworkURL = function() return "https://example.com/artwork.jpg" end,
    ...
    local fake_hs = {
      ...
      image = {
        imageFromURL = function(url, callback)
          assertEqual(url, "https://example.com/artwork.jpg")
    ...
    local button = context("spotify-button")
    button.getDevice = function()
      return { controllerType = "keypad", device = { type = "stream-deck-plus", size = { columns = 4, rows = 2 } } }
    end
    local volume_dial = context("spotify-volume", { dialControl = "volume" })
    volume_dial.getDevice = function()
      return { controllerType = "encoder", device = { type = "stream-deck-plus", size = { columns = 4, rows = 2 } } }
    end
    local track_dial = context("spotify-tracks", { dialControl = "tracks" })
    track_dial.getDevice = volume_dial.getDevice
    ...
    local png = "iVBORw0KGgoAAAANSUhEUgAAAEgAAABICAYAAABV7bNHAAAAK0lEQVR4nO3BAQ0AAADCoPdPbQ43oAAAAAAAAAAAAAAAAAAAAAAAAAAAPgxRSAABHLYB8AAAAABJRU5ErkJggg=="
    artwork_callback({
    ...
    assertEqual(artwork.mediaType, "image/png")
    assertEqual(artwork.dataBase64, png)
    ...
    assertEqual(track_appearance.icon.dataBase64, png)
