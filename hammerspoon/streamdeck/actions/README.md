...
| ---------------------- | ---------------------------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `application`          | `com.brettinternet.hammerspoon.application-toggle`   | Toggle         | Opens an app that is not running; otherwise hides, shows, focuses, or closes it. When focus on show is enabled, an unfocused visible app is focused first and hiding restores the prior app. Shows the macOS icon with green, yellow, or red window-state indicator; without a bundle ID it follows the frontmost application. |
| `app-windows-to-cursor` | `com.brettinternet.hammerspoon.app-windows-to-cursor` | Button         | Moves every window of the frontmost app to the display under the cursor, preserving each window's relative placement. Requires Accessibility permission. |
| `audio-input-router`   | `com.brettinternet.hammerspoon.audio-input-router`   | Multi-State / Dial | Keys cycle up to four inputs; dials preview inputs while rotating and switch on press. External mute changes refresh the icon; disconnected choices are skipped and rejoin automatically. |
...
| `command` (opt-in)     | `com.brettinternet.hammerspoon.command`              | Button         | Runs an absolute executable path with a shell-style argument string, directly through `hs.task` without a shell. Quotes and backslashes preserve spaces; expansion, pipes, and redirects are not supported. Defaults to `/usr/bin/pmset displaysleepnow` to sleep the displays without sleeping the Mac. |
...
| `microphone`           | `com.brettinternet.hammerspoon.microphone-toggle`    | Toggle / Pedal | Toggles a selected input, provides push-to-talk, or combines tap-to-toggle with hold-to-talk; follows external input mute changes and can send per-app Zoom, Teams, Slack, and Discord mute shortcuts. |
...
| `system-monitor`       | `com.brettinternet.hammerspoon.system-monitor`       | Button         | Displays a live 120-second chart for CPU, memory, root-disk usage, primary-network status, thermal state, or idle time. CPU, memory, and memory-pressure keys open Activity Monitor; network opens Network settings. Other metrics do not act on press. |
...
| `url-toggle`           | `com.brettinternet.hammerspoon.url-toggle`           | Button         | Opens the configured URL in a new Chromium window by default, or a new tab in the frontmost window when configured; closes its existing tab, uses the site's favicon when available, and shows green/open or red/closed status. Requires Chromium automation permission. |
